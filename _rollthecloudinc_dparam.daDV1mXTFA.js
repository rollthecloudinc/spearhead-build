...
import { Plugin as R, BasePluginManager as U, PluginDef as z } from "@rollthecloudinc/plugin";
import { of as c, iif as y, BehaviorSubject as d, Subject as O, combineLatest as j, filter as K, tap as J, forkJoin as N } from "rxjs";
import { map as m, switchMap as F, tap as g, debounceTime as D, filter as H, delay as Q, defaultIfEmpty as W } from "rxjs/operators";
import * as a from "@angular/forms";
import { Validators as G, NG_VALUE_ACCESSOR as M, NG_VALIDATORS as w, FormsModule as X, ReactiveFormsModule as Y } from "@angular/forms";
...
var b = class extends R {
    ...
    pluginDef() { return c(new z({ name: "param" })); }
    ...
    constructor(t, i) { this.fb = t, this.ppm = i, this.contexts = [], this.params$ = new d({}), this.paramValues$ = new d([]), this.init$ = new O, this.formArray = this.fb.array([]), this.paramPlugins$ = this.ppm.getPlugins(), this.flags = new Map, this.savedParams = new Map, this.paramIndexes = new Map, this.paramsSub = this.params$.pipe(g(n => { let o = new Map([...this.paramIndexes]); this.formArray.clear(), this.paramIndexes.clear(); let l = 0; for (let f in n)
        ...
        let f = Array.from(this.paramIndexes).find(([I, L]) => L === l);
    ...
    constructor(t) { this.fb = t, this.contexts = [], this.settings$ = new d(void 0), this.afterViewInit$ = new O, this.paramValues$ = new d([]), this.formGroup = this.fb.group({ paramsString: this.fb.control(""), params: this.fb.control([]) }), this.paramsStringChangeSub = this.formGroup.get("paramsString").valueChanges.pipe(D(500)).subscribe(i => { let n = q.parse("?" + i); this.paramsParsed = n; }), this.settingsSub = this.settings$.pipe(g(i => { i && this.formGroup.get("paramsString").setValue(i.paramsString ? i.paramsString : ""); })).subscribe(), this.settingsParamsSub = j([this.settings$, this.formGroup.get("paramsString").valueChanges]).pipe(H(([i]) => i !== void 0), Q(1), g(([i]) => this.paramValues$.next(i.params))).subscribe(), this.onTouched = () => { }; }
    ...
    constructor(t) { this.fb = t, this.plugins$ = new d([]), this.title$ = new d("Plugin"), this.instance$ = new d(new V), this.instanceForm = this.fb.group({ plugin: this.fb.control(""), settings: this.fb.control("") }), this.instanceSub = this.instance$.pipe(K(i => !!i), J(i => { this.instanceForm.get("plugin").setValue(i.plugin); })).subscribe(), this.onTouched = () => { }; }
    ...
    paramValues(t) { return N(Array.from(t.keys()).map(i => this.paramValue(t.get(i), new Map).pipe(m(n => [i, n])))).pipe(m(i => i.reduce((n, o) => new Map([...n, o]), new Map)), W(new Map)); }
    ...
    static { this.\u0275fac = function (i) { return new (i || r)(e.\u0275\u0275inject(_), e.\u0275\u0275inject(B.TokenizerService)); }; }
    static { this.\u0275prov = e.\u0275\u0275defineInjectable({ token: r, factory: r.\u0275fac, providedIn: "root" }); }
} return r; })(), Ce = (() => { class r {
    ...
    static { this.\u0275inj = e.\u0275\u0275defineInjector({ imports: [Z, X, Y, ee] }); }
} return r; })();
export { Ce as DparamModule, C as Flag, S as Mapping, v as Param, ue as ParamEvaluatorService, b as ParamPlugin, V as ParamPluginInstance, _ as ParamPluginManager, $ as ParamSettings, pe as ParamsFormComponent, me as ParamsQuerystringComponent, Se as PluginInstanceComponent };
//# sourceMappingURL=_rollthecloudinc_dparam.daDV1mXTFA.js.map
