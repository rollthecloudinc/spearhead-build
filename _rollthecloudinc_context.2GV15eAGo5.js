...
import { InjectionToken as re, forwardRef as x } from "@angular/core";
...
function Ie(n, r) { }
var I = new re("ContextPlugin"), w = class extends U {
    ...
    constructor(r) { this.name = r.name, this.adaptor = r.adaptor, r.plugin && (this.plugin = r.plugin), this.adaptor === "rest" ? this.rest = new me(r.rest) : this.adaptor === "snippet" || this.adaptor === "json" ? this.snippet = new pe(r.snippet) : this.adaptor === "data" ? this.data = r.data : this.adaptor === "token" ? this.tokens = new Map([...r.tokens]) : this.adaptor === "datasource" && (this.datasource = new F(r.datasource)); }
}, C = class {
    ...
    static { this.\u0275cmp = i.\u0275\u0275defineComponent({ type: n, selectors: [["classifieds-ui-datasource-context-editor"]], viewQuery: function (t, o) { if (t & 1 && i.\u0275\u0275viewQuery(fe, 7), t & 2) {
            let s;
...
} return n; })(), Oe = n => { let r = { path: "", arg0: "", arg2: "", arg3: "", arg4: "", arg5: "" }; return new w({ id: "route", name: "route", title: "Route", global: !0, baseObject: r, resolver: n }); }, Ee = (n, r) => new ve({ id: "context", title: "Context", build: () => { k.prototype.resolveContext = (e, t) => new Promise(o => { n.getPlugin(e).pipe(u(s => s.resolver.resolve(s, t).pipe(h(1)))).subscribe(s => { o(s); }); }), k.prototype.resolveMergedContext = e => new Promise(t => { r.resolveMerged(e).pipe(X(o => t(o)), h(1)).subscribe(); }); } }), Ne = (n, r) => new be({ id: "context", title: "Context", evalParam: ({ param: e, metadata: t }) => { let o = e.mapping.context && e.mapping.context !== "" ? new A(t.get("contexts").find(s => s.name === e.mapping.context)) : void 0; return o ? n.resolve(o).pipe(h(1), u(s => Y(() => e.mapping.value && e.mapping.value !== "", p(s).pipe(c(l => r.generateGenericTokens(Array.isArray(l) ? l[0] : l)), c(l => r.replaceTokens(`[${e.mapping.value}]`, l)), h(1)), p(Array.isArray(s) ? s[0] : s)).pipe(h(1)))) : p(); }, usedContexts: ({ param: e, metadata: t }) => p([e.mapping.context]) }), Te = (n, r) => new ge({ id: "context", title: "Context", editor: Ge, fetch: ({ settings: e, metadata: t }) => p(new v).pipe(c(() => new C(r.deserializeAsObject(e))), c(o => t.get("contexts").find(s => s.name === o.name)), u(o => n.resolve(o).pipe(c(s => s instanceof v ? s : new v({ results: Array.isArray(s) ? s : [s] })), h(1)))), getBindings: ({ settings: e, metadata: t }) => p([]).pipe(c(() => new C(r.deserializeAsObject(e))), c(o => [new we({ id: o.name, type: "context" })])) }), _e = n => { let r = { dataset: new v }; return new w({ id: "datasource", name: "datasource", title: "Datasource", baseObject: r, resolver: n, editorComponent: $e }); }, D = (() => { class n {
    ...
    resolveMerged(r, e = d()) { return this.rcm.getPlugins().pipe(u(() => this.rcm.add$.pipe(Ce(void 0))), u(() => this.rcm.getPlugins()), u(t => xe([...Array.from(t).map(([o, s]) => s.resolve()), this.resolveGlobals(e), r.length === 0 ? p({}) : this.resolveAll(r, e)]).pipe(ye(0), c(o => o.reduce((s, l) => b(b({}, s), l), {}))))); }
    ...
    constructor(e, t, o, s, l, ee, te, ie, ne) { e.forEach(oe => s.register(oe)), l.register(Ee(s, t)), te.register(Ne(t, o)), ee.register(Te(t, ie)), s.register(_e(ne)); }
    ...
    extractContexts(e) { return Fe(e.map(t => this.ppm.getPlugin(t.mapping.type).pipe(u(o => Y(() => !!o && !!o.usedContexts, o ? o.usedContexts ? o.usedContexts({ param: t, metadata: new Map([]) }) : p([]) : p([]), p([])))))).pipe(c(t => t.reduce((o, s) => [...o, ...s.filter(l => !o.includes(l))], []))); }
...
//# sourceMappingURL=_rollthecloudinc_context.2GV15eAGo5.js.map
