...
import { InjectionToken as x, inject as f, DOCUMENT as G, ElementRef as J, ErrorHandler as K, HostAttributeToken as Q, booleanAttribute as X } from "@angular/core";
import { Subscription as Z } from "rxjs";
import { take as tt } from "rxjs/operators";
import * as M from "@angular/common/http";
import { HttpClient as D } from "@angular/common/http";
import * as h from "@angular/core";
import { SecurityContext as d, DOCUMENT as A, Optional as F, SkipSelf as U, ErrorHandler as H } from "@angular/core";
import * as b from "@angular/platform-browser";
import { DomSanitizer as z } from "@angular/platform-browser";
import { of as g, throwError as k, forkJoin as W } from "rxjs";
import { tap as w, map as _, catchError as $, finalize as j, share as V } from "rxjs/operators";
var S;
function B() { if (S === void 0 && (S = null, typeof window < "u")) {
    let r = window;
    r.trustedTypes !== void 0 && (S = r.trustedTypes.createPolicy("angular#components", { createHTML: c => c }));
} return S; }
function p(r) { return B()?.createHTML(r) || r; }
function R(r) { return Error(`Unable to find icon with the name "${r}"`); }
function L() { return Error("Could not find HttpClient for use with Angular Material icons. Please add provideHttpClient() to your providers."); }
function y(r) { return Error(`The URL provided to MatIconRegistry was not trusted as a resource URL via Angular's DomSanitizer. Attempted URL was "${r}".`); }
function N(r) { return Error(`The literal provided to MatIconRegistry was not trusted as safe HTML by Angular's DomSanitizer. Attempted literal was "${r}".`); }
var l = class {
    url;
    svgText;
    options;
    ...
    addSvgIconInNamespace(t, e, n, o) { return this._addSvgIconConfig(t, e, new l(n, null, o)); }
    addSvgIconResolver(t) { return this._resolvers.push(t), this; }
    addSvgIconLiteralInNamespace(t, e, n, o) { let i = this._sanitizer.sanitize(d.HTML, n); if (!i)
        throw N(n); let s = p(i); return this._addSvgIconConfig(t, e, new l("", s, o)); }
    ...
    addSvgIconSetLiteralInNamespace(t, e, n) { let o = this._sanitizer.sanitize(d.HTML, e); if (!o)
        throw N(e); let i = p(o); return this._addSvgIconSetConfig(t, new l("", i, n)); }
    ...
    getSvgIconFromUrl(t) { let e = this._sanitizer.sanitize(d.RESOURCE_URL, t); if (!e)
        throw y(t); let n = this._cachedIconsByUrl.get(e); return n ? g(C(n)) : this._loadSvgIconFromConfig(new l(t, null)).pipe(w(o => this._cachedIconsByUrl.set(e, o)), _(o => C(o))); }
    getNamedSvgIcon(t, e = "") { let n = T(e, t), o = this._svgIconConfigs.get(n); if (o)
        return this._getSvgFromConfig(o); if (o = this._getIconConfigFromResolvers(e, t), o)
        return this._svgIconConfigs.set(n, o), this._getSvgFromConfig(o); let i = this._iconSetConfigs.get(e); return i ? this._getSvgFromIconSetConfigs(t, i) : k(R(n)); }
    ngOnDestroy() { this._resolvers = [], this._svgIconConfigs.clear(), this._iconSetConfigs.clear(), this._cachedIconsByUrl.clear(); }
    _getSvgFromConfig(t) { return t.svgText ? g(C(this._svgElementFromConfig(t))) : this._loadSvgIconFromConfig(t).pipe(_(e => C(e))); }
    _getSvgFromIconSetConfigs(t, e) { let n = this._extractIconWithNameFromAnySet(t, e); if (n)
        return g(n); let o = e.filter(i => !i.svgText).map(i => this._loadSvgIconSetFromConfig(i).pipe($(s => { let m = `Loading icon set URL: ${this._sanitizer.sanitize(d.RESOURCE_URL, i.url)} failed: ${s.message}`; return this._errorHandler.handleError(new Error(m)), g(null); }))); return W(o).pipe(_(() => { let i = this._extractIconWithNameFromAnySet(t, e); if (!i)
        throw R(t); return i; })); }
    _extractIconWithNameFromAnySet(t, e) { for (let n = e.length - 1; n >= 0; n--) {
        let o = e[n];
        if (o.svgText && o.svgText.toString().indexOf(t) > -1) {
            let i = this._svgElementFromConfig(o), s = this._extractSvgIconFromSet(i, t, o.options);
            if (s)
                return s;
        }
    } return null; }
    _loadSvgIconFromConfig(t) { return this._fetchIcon(t).pipe(w(e => t.svgText = e), _(() => this._svgElementFromConfig(t))); }
    _loadSvgIconSetFromConfig(t) { return t.svgText ? g(null) : this._fetchIcon(t).pipe(w(e => t.svgText = e)); }
    _extractSvgIconFromSet(t, e, n) { let o = t.querySelector(`[id="${e}"]`); if (!o)
        return null; let i = o.cloneNode(!0); if (i.removeAttribute("id"), i.nodeName.toLowerCase() === "svg")
        return this._setSvgAttributes(i, n); if (i.nodeName.toLowerCase() === "symbol")
        return this._setSvgAttributes(this._toSvgElement(i), n); let s = this._svgElementFromString(p("<svg></svg>")); return s.appendChild(i), this._setSvgAttributes(s, n); }
    ...
    _toSvgElement(t) { let e = this._svgElementFromString(p("<svg></svg>")), n = t.attributes; for (let o = 0; o < n.length; o++) {
        let { name: i, value: s } = n[o];
        i !== "id" && e.setAttribute(i, s);
    ...
    _fetchIcon(t) { let { url: e, options: n } = t, o = n?.withCredentials ?? !1; if (!this._httpClient)
        throw L(); if (e == null)
        throw Error(`Cannot fetch icon from URL "${e}".`); let i = this._sanitizer.sanitize(d.RESOURCE_URL, e); if (!i)
        throw y(e); let s = this._inProgressUrlFetches.get(i); if (s)
        return s; let u = this._httpClient.get(i, { responseType: "text", withCredentials: o }).pipe(_(m => p(m)), j(() => this._inProgressUrlFetches.delete(i)), V()); return this._inProgressUrlFetches.set(i, u), u; }
    _addSvgIconConfig(t, e, n) { return this._svgIconConfigs.set(T(t, e), n), this; }
    ...
    _getIconConfigFromResolvers(t, e) { for (let n = 0; n < this._resolvers.length; n++) {
        ...
        if (o)
            return q(o) ? new l(o.url, null, o.options) : new l(o, null);
    } }
    static \u0275fac = function (e) { return new (e || r)(h.\u0275\u0275inject(M.HttpClient, 8), h.\u0275\u0275inject(b.DomSanitizer), h.\u0275\u0275inject(A, 8), h.\u0275\u0275inject(h.ErrorHandler)); };
    static \u0275prov = h.\u0275\u0275defineInjectable({ token: r, factory: r.\u0275fac, providedIn: "root" });
} return r; })();
function O(r, c, t, e, n) { return r || new I(c, t, n, e); }
var Y = { provide: I, deps: [[new F, new U, I], [new F, D], z, H, [new F, A]], useFactory: O };
function C(r) { return r.cloneNode(!0); }
function T(r, c) { return r + ":" + c; }
function q(r) { return !!(r.url && r.options); }
import "@angular/common/http";
import "@angular/platform-browser";
import "@angular/cdk/a11y";
import "@angular/cdk/bidi";
var et = ["*"], nt = new x("MAT_ICON_DEFAULT_OPTIONS"), ot = new x("mat-icon-location", { providedIn: "root", factory: rt });
function rt() { let r = f(G), c = r ? r.location : null; return { getPathname: () => c ? c.pathname + c.search : "" }; }
var P = ["clip-path", "color-profile", "src", "cursor", "fill", "filter", "marker", "marker-start", "marker-mid", "marker-end", "mask", "stroke"], it = P.map(r => `[${r}]`).join(", "), st = /^url\(['"]?#(.*?)['"]?\)$/, Mt = (() => {
    class r {
        _elementRef = f(J);
        _iconRegistry = f(I);
        _location = f(ot);
        ...
        _currentIconFetch = Z.EMPTY;
        constructor() { let t = f(new Q("aria-hidden"), { optional: !0 }), e = f(nt, { optional: !0 }); e && (e.color && (this.color = this._defaultColor = e.color), e.fontSet && (this.fontSet = e.fontSet)), t || this._elementRef.nativeElement.setAttribute("aria-hidden", "true"); }
        ...
        _prependPathToReferences(t) { let e = this._elementsWithExternalReferences; e && e.forEach((n, o) => { n.forEach(i => { o.setAttribute(i.name, `url('${t}#${i.value}')`); }); }); }
        _cacheChildrenWithExternalReferences(t) { let e = t.querySelectorAll(it), n = this._elementsWithExternalReferences = this._elementsWithExternalReferences || new Map; for (let o = 0; o < e.length; o++)
            P.forEach(i => { let s = e[o], u = s.getAttribute(i), m = u ? u.match(st) : null; if (m) {
                let v = n.get(s);
                v || (v = [], n.set(s, v)), v.push({ name: i, value: m[1] });
            } }); }
...
export { R as a, L as b, y as c, N as d, I as e, O as f, Y as g, nt as h, ot as i, rt as j, Mt as k, At as l };
//# sourceMappingURL=chunk-ADQVIUAN.js.map
