import { a as h, b as C } from "@nf-internal/chunk-GL2BOVXA";
...
import * as T from "angular-split";
...
import * as O from "@rollthecloudinc/snippet";
...
import { BehaviorSubject as k, Subject as q, of as y, combineLatest as he, Observable as ge, iif as ye, throwError as ve } from "rxjs";
import { map as d, tap as w, filter as N, switchMap as g, catchError as U, takeUntil as Se, debounceTime as P, delay as F, timeout as we } from "rxjs/operators";
import * as $ from "qs";
...
var b = class {
    ...
    constructor(i, o, t) { this.fb = i, this.datasourceApi = o, this.tokenizerService = t, this.dataChange = new ie, this.contexts = [], this.sourceForm = this.fb.group({ url: this.fb.control("", V.required), params: this.fb.control([]), body: this.fb.control(""), method: this.fb.control("get", [V.required]) }), this.restSource$ = new k(new b), this.urlParams$ = new k({}), this.bodyParams$ = new k({}), this.componentDestroyed = new q, this.refreshData$ = new q, this.refreshSubscription = this.refreshData$.pipe(d(() => this.generateUrl()), w(e => console.log("URL Generated:", e)), N(e => e && e.trim() !== ""), g(e => { let r = this.sourceForm.get("method").value, n = this.sourceForm.get("body").value?.content; return (r === "post" ? this.datasourceApi.postData({ url: e, body: n }) : this.datasourceApi.getData(e)).pipe(U(u => (console.error(u), y([])))); }), Se(this.componentDestroyed)).subscribe(e => { this.jsonData = e, this.dataChange.emit(e); }), this.urlChangeSub = this.sourceForm.get("url").valueChanges.pipe(P(500), N(e => e && e.trim() !== ""), d(e => [e, e.indexOf("?")]), d(([e, r]) => [r > -1 ? e.substring(0, r) : e, r > -1 ? e.substring(r + 1) : ""])).subscribe(([e, r]) => { let n = e.split("/").reduce((u, l, v) => l.indexOf(":") === 0 ? C(h({}, u), { [l.substr(1)]: l }) : u, {}), c = h(h({}, n), $.parse(r)); console.log("params parsed", c), this.urlParams$.next(c); }), this.bodyChangeSub = this.sourceForm.get("body").valueChanges.pipe(P(500), w(e => { console.log("body change", e); let r = this.parseBody(e.content); this.bodyParams$.next(h({}, r)); })).subscribe(), this.paramsChangesSub = he([this.urlParams$, this.bodyParams$]).pipe(P(500), w(([e, r]) => { this.paramsParsed = h(h({}, e), r); })).subscribe(), this.restSourceSub = this.restSource$.pipe(w(e => { e ? (this.sourceForm.get("url").setValue(e.url), this.sourceForm.get("method").setValue(e.method ? e.method : ""), this.sourceForm.get("body").setValue(e.body ? C(h({}, e.body), { jsScript: "" }) : "")) : (this.sourceForm.get("url").setValue(""), this.sourceForm.get("method").setValue(""), this.sourceForm.get("body").setValue("")); })).subscribe(), this.onTouched = () => { }; }
    ngOnInit() { this.sourceForm.valueChanges.pipe(P(1e3)).subscribe(() => { console.log("refresh data"), this.refreshData$.next(void 0); }); }
    ...
    generateUrl() { console.log("generate url"); let i = this.sourceForm.get("url").value, [o, t] = i ? i.split("?", 2) : ["", ""], e = $.parse(t), r = {}, n = o.split("/"), c = n.length, u = [], l = 0; for (let f = 0; f < c; f++)
        if (n[f].indexOf(":") === 0) {
    ...
    static { this.\u0275fac = function (o) { return new (o || a)(s.\u0275\u0275directiveInject(A.AttributeSerializerService), s.\u0275\u0275directiveInject(m.ControlContainer)); }; }
    ...
    case "DELETE": {
        n = e.delete(p, o), t.has("saveDelay") && (n = n.pipe(F(+t.get("saveDelay"))));
        break;
    }
    case "GET": {
        let c = t.has("suffix") ? p + t.get("suffix") : p;
        n = e.get(c, o), t.has("getDelay") && (n = n.pipe(F(+t.get("getDelay"))));
        break;
    }
    case "POST": {
        n = e.post(p, i, o), t.has("saveDelay") && (n = n.pipe(F(+t.get("saveDelay"))));
        break;
    }
    case "PUT": {
        n = e.put(p, i, o), t.has("saveDelay") && (n = n.pipe(F(+t.get("saveDelay"))));
        break;
    }
    default: {
...
} return t.has("timeout") && (n = n.pipe(we(+t.get("timeout") + +t.get("saveDelay")))), n.pipe(U(() => y([]))); }, G = (() => { class a {
    ...
    static { this.\u0275fac = function (o) { return new (o || a)(s.\u0275\u0275inject(A.AttributeSerializerService), s.\u0275\u0275inject(z.HttpClient), s.\u0275\u0275inject(O.SnippetParserService), s.\u0275\u0275inject(Q.UrlGeneratorService), s.\u0275\u0275inject(x.ParamEvaluatorService)); }; }
    ...
    static { this.\u0275fac = function (o) { return new (o || a)(s.\u0275\u0275inject(R.Pluralizer)); }; }
    static { this.\u0275prov = s.\u0275\u0275defineInjectable({ token: a, factory: a.\u0275fac }); }
} return a; })(), tt = (() => { class a {
    constructor(i, o, t, e, r, n, c, u, l) { i.register(Ae(o, t, e)), r.register(Re(n, c, u, l)); }
    static { this.\u0275fac = function (o) { return new (o || a)(s.\u0275\u0275inject(E.DatasourcePluginManager), s.\u0275\u0275inject(G), s.\u0275\u0275inject(W.ParamContextExtractorService), s.\u0275\u0275inject(A.AttributeSerializerService), s.\u0275\u0275inject(Z.CrudAdaptorPluginManager), s.\u0275\u0275inject(x.ParamEvaluatorService), s.\u0275\u0275inject(z.HttpClient), s.\u0275\u0275inject(B), s.\u0275\u0275inject(R.DefaultDataServiceConfig)); }; }
    static { this.\u0275mod = s.\u0275\u0275defineNgModule({ type: a }); }
    static { this.\u0275inj = s.\u0275\u0275defineInjector({ providers: [G, B], imports: [X, te, re, se, ue, pe, de, le, fe, xe, Ce] }); }
} return a; })();
export { b as Rest, Me as RestDatasourceComponent, tt as RestModule, Oe as RestSourceFormComponent };
//# sourceMappingURL=_rollthecloudinc_rest.7ms0WFjq3U.js.map
