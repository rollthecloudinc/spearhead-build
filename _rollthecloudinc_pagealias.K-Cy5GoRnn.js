...
import { createMatcher as w, EditPanelPageComponent as L, createEditMatcher as S } from "@rollthecloudinc/pages";
import { PanelPageRouterComponent as E } from "@rollthecloudinc/render";
...
var c = class extends C {
    ...
    load() { return this.panelPageListItemsService.getWithQuery(`site=${encodeURIComponent(`{"term":{"site.keyword":{"value":"${this.siteName}"}}}`)}&path=${encodeURIComponent('{"wildcard":{"path.keyword":{"value":"*"}}}')}`).pipe(l(t => t.filter(e => e.path !== void 0 && e.path !== "")), l(t => t.map(e => new $(e)).sort((e, i) => e.path.split("/").length === i.path.split("/").length ? e.path.split("/")[e.path.split("/").length - 1] > i.path.split("/")[i.path.split("/").length - 1] ? -1 : 1 : e.path.split("/").length > i.path.split("/").length ? -1 : 1)), h(t => t.sort((e, i) => e.path.length > i.path.length ? 1 : -1)), h(t => { t.forEach(e => { this.router.config.unshift({ matcher: w(e), component: E, data: { panelPageListItem: e } }), this.router.config.unshift({ matcher: S(e), component: L, data: { panelPageListItem: e } }); }), this.routesLoaded = !0; }), h(() => console.log("panels routes loaded")), l(() => !0)); }
}, m = class {
    get panelPageListItemsService() { return this.es.getEntityCollectionService("PanelPageListItem"); }
    ...
    encodePathComponent(t) { return `{"term":{"path.keyword":{"value":"${t}"}}}`; }
}, j = (a, t, e) => { let i = new u(a, t, e), o = new m(a, t, e), n = new d(a, t, e); return new c({ id: "panels", title: "Panels", loadingStrategy: i, matchingStrategy: o, redirectHandler: n }); }, T = (() => { class a {
...
export { T as PagealiasModule };
//# sourceMappingURL=_rollthecloudinc_pagealias.K-Cy5GoRnn.js.map
