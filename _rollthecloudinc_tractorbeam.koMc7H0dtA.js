...
import { Validators as l, NG_VALUE_ACCESSOR as I, NG_VALIDATORS as G, FormsModule as D, ReactiveFormsModule as w } from "@angular/forms";
...
import { loadRemoteModule as u } from "@softarc/native-federation-runtime";
...
var E = (() => { class i {
    ...
    constructor(t, o) { this.fb = t, this.controlContainer = o, this.context$ = new q(void 0), this.formGroup = this.fb.group({ remoteEntry: this.fb.control("", [l.required]), exposedModule: this.fb.control("", [l.required]), moduleName: this.fb.control("", [l.required]) }), this.onTouched = () => { }; }
    ...
    ngOnInit() { this.controlContainer.control.addControl("adaptor", this.fb.control("data", l.required)), this.controlContainer.control.addControl("data", this.fb.control("")); }
    static { this.\u0275fac = function (o) { return new (o || i)(e.\u0275\u0275directiveInject(r.UntypedFormBuilder), e.\u0275\u0275directiveInject(r.ControlContainer)); }; }
    ...
    constructor(t, o, a) { this.actions$ = t, this.moduleLoaderService = o, this.es = a, this.setPage$ = v(() => this.actions$.pipe(g(T), f(({ page: n }) => { console.log("tractor beam capture page", n), n.contexts && Array.isArray(n.contexts) && n.contexts.forEach(d => { if (d.plugin === "module")
        return this.moduleLoaderService.loadModule(() => u({ remoteEntry: d.data.remoteEntry, remoteName: "plugin", exposedModule: d.data.exposedModule }).then(c => c[d.data.moduleName])).subscribe(); }); })), { dispatch: !1 }), this.setPageInfo$ = v(() => this.actions$.pipe(g(O), f(({ info: n }) => { console.log("tractor beam info", n); }), B(({ info: n }) => this.es.getEntityCollectionService("PanelPage").getByKey(n.id)), f(n => { n.contexts && Array.isArray(n.contexts) && n.contexts.forEach(d => { if (d.plugin === "module")
        return this.moduleLoaderService.loadModule(() => u({ remoteEntry: d.data.remoteEntry, remoteName: "plugin", exposedModule: d.data.exposedModule }).then(c => c[d.data.moduleName])).subscribe(); }); })), { dispatch: !1 }); }
    ...
    resolve(t, o, a) { return console.log("module resolver context", t, o, a), this.moduleLoaderService.loadModule(() => u({ remoteEntry: o.remoteEntry, remoteName: "plugin", exposedModule: o.exposedModule }).then(n => n[o.moduleName])); }
    ...
    loadPlugins() { return this.pageBuilderFacade.getPageInfo$.pipe(M(() => console.log("started loading external modules")), x(t => (t && t.id ? this.es.getEntityCollectionService("PanelPage").getByKey(t.id) : y(void 0)).pipe(C(void 0))), x(t => t ? R(t.contexts.filter(o => o.plugin === "module").map(o => this.moduleLoader.loadModule(() => u({ remoteEntry: o.data.remoteEntry, remoteName: "plugin", exposedModule: o.data.exposedModule }).then(a => a[o.data.moduleName])))).pipe(k(1), C([])) : y([])), M(() => console.log("completed loading external modules")), Q(() => !0), U(1)); }
...
//# sourceMappingURL=_rollthecloudinc_tractorbeam.koMc7H0dtA.js.map
